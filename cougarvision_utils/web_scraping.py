...
#get photos from specific camera filtered by date
#(will need to loop through pages)
request <- "photos"
parameters <- "page=1&sort_date=desc&camera_id[]=60272&date_start=2022-09-01
&date_end=2022-10-07"
...
# 5 second delay between captures, maximum 12 photos between checks
    ...
    for i in range(len(photos)):
        if int(photos[i]['id']) > last_id:
            ...
            print(info)
            camera = camera_names[photos[i]['relationships']['camera']['data']
                                  ['id']]
            newname = config['save_dir'] + camera + "_"
            newname += info['file_thumb_filename']
            urllib.request.urlretrieve(info['file_thumb_url'], newname)
            new_photos.append([photos[i]['id'], info['file_thumb_url'],
                               newname])
...
def fetch_images(config_path):
    ...
    driver = webdriver.Chrome(ChromeDriverManager(chrome_type
=ChromeType.CHROMIUM).install(), options=options)
    ...
    df = pd.DataFrame(
        columns=['file', 'camera_name', 'time', 'date', 'temperature', 'moon',
 'camera_id', 'alt', 'image_id', 'src'])
    skip = True
    first = True
    i = 0
    while i < image_num:
...
'body').send_keys(Keys.PAGE_DOWN)
 ...
 document.body.scrollHeight")
                ...
                if last_id is not None:
                    image_urls = driver.find_elements(By.CLASS_NAME,
 'css-1vh28r')
                    # make a list of photo ids sorted from greatest to least
                    photo_ids = [x.get_attribute('data-photo-id') for x
 in image_urls]
                    photo_ids.sort(reverse=True)
            ...
            driver.find_element_by_xpath(
                '/html/body/div/section/main/div/div[2]/div/div/div/div/div[2]
/div[1]/div/img').click()
            time.sleep(2)
        ...
        datetime_object = datetime.datetime.strptime(time_stamp,
'%I:%M%p') + datetime.timedelta(hours=2)
        time_stamp = datetime_object.strftime("%H:%M:%S")
        temp_moon_stamps = driver.find_elements(By.CLASS_NAME, 'css-9kmbgq')
        temp_stamp, moon_stamp = temp_moon_stamps[0].text,
 temp_moon_stamps[1].text
        ...
        camera_names = {'62201': 'B018', '59760': 'B016', '59681': 'B013',
 '59758': 'B015', '60095': 'B014',
                        '60272': 'B017'}
        camera_name = camera_names[camera_id]
        # Download image
        path = f'{image_path}{camera_name}_{date_stamp.replace("/",
 "-")}_{picture_id}.png'
        urllib.request.urlretrieve(src, path)

        # add all information to dataframe
        df.loc[len(df.index)] = [path, camera_name, time_stamp, date_stamp,
 temp_stamp, moon_stamp, camera_id, alt,
                                 picture_id, src]
        i += 1
    driver.close()
    return df
